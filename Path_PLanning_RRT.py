...
class Obstacle:
    def __init__(self):
        ...
        if self.x + self.w > plot_width:
            self.w = plot_width - self.x
        
        if self.y + self.h > plot_height:
            self.h = plot_height - self.y
...
class Node(Point):
    def __init__(self):
        Point.__init__(self, randrange(5, 95), randrange(5, 95))


class RRT:
    def __init__(self, n_obs, steps, max_step_size):
    ...
    def find_nearest_neighbor(self, point):
        ...
        if d <= self.max_step_size:
            ...
            if x2 != x1:
                ...
                new_x = x1 + (self.max_step_size*cos_theta)
                new_y = y1 + (self.max_step_size*sin_theta)
